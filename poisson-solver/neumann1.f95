!gfortran neumann1.f95 -fimplicit-none -O3 -o neumann1 && ./neumann1
      ...
      FUNCTION BOUNDARYTEST(X,Y,H,PHIS,LENGTH)
        ...
        ELSE IF (Y==LENGTH) THEN
          ...
          BOUNDARYTEST = (PHIS(X,Y-1) + NEWVALUE)/2.0
        ELSE
          BOUNDARYTEST = NEWESTIMATE(X,Y,H,PHIS,LENGTH)
        END IF
        RETURN
      END FUNCTION
      ...
      FUNCTION BOUNDARYTESTT(X,Y,H,PHIS,LENGTH)
        ...
        ELSE IF (Y==LENGTH) THEN
          ...
          BOUNDARYTESTT = NEWVALUE
        ELSE
          BOUNDARYTESTT = NEWESTIMATE(X,Y,H,PHIS,LENGTH)
        END IF
        RETURN
      END FUNCTION
      ...
      PROGRAM SOLVER
        ...
        NUMITERATIONS = 0
        DO WHILE (ANALERROR(PHIS,LENGTH,H,SIZE)>0.5) !((NEWESTIMATE(2,2,H,PHIS,LENGTH)-0)**2>0.00001) !((LOWHIGHESTCHANGE/HIGHESTCHANGE) > 1.0 .OR. (LOWAVGCHANGE/AVGCHANGE) > 1.0) !(NUMITERATIONS < 200000)
