!gfortran neumann13.f95 -fimplicit-none -O3 -o neumann13 && time ./neumann13
      ...
      FUNCTION BOUNDARY(X,Y,H,PHIS,LENGTH)
        ...
        ELSE IF (X==1 .AND. Y==LENGTH) THEN
          BOUNDARY = (1.0/4.0)*(2*PHIS(X+1,Y)+2*PHIS(X,Y-1)-H*H*F1(X,Y,H))
        ELSE IF (X==LENGTH .AND. Y==1) THEN
          BOUNDARY = (1.0/4.0)*(2*PHIS(X,Y+1)+2*PHIS(X-1,Y)-H*H*F1(X,Y,H))
