!gfortran neumann127.f95 -fimplicit-none -O3 -o neumann127 -fdefault-real-8 && time ./neumann127
      ...
      FUNCTION BOUNDARY(X,Y,H,PHIS,LENGTH)
        ...
        ELSE IF (X==1 .AND. Y==LENGTH) THEN
          BOUNDARY = (1.0/2.0)*(PHIS(X+1,Y)+PHIS(X,Y-1)-H*H*F1(X,Y,H))
      ...
      PROGRAM SOLVER
        ...
        NUMITERATIONS = 0
        DO WHILE (ANALERROR(PHIS,LENGTH,H,SIZE)>0.1) !((NEWESTIMATE(2,2,H,PHIS,LENGTH)-0)**2>0.00001) !((LOWHIGHESTCHANGE/HIGHESTCHANGE) > 1.0 .OR. (LOWAVGCHANGE/AVGCHANGE) > 1.0) !(NUMITERATIONS < 200000)
